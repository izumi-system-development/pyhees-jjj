...
def get_V_hs_vent_d_t(
    ...
    assert V_vent_g_i.shape == (5,), "想定外の行列数"
    ...
    if general_ventilation == True:
        if input_V_hs_min == 最低風量直接入力.入力する.value:  # 最低風量直接入力あり
            ...
            V_hs_vent_d_t[H & (V_vent_g < get_V_hs_min_H(V_vent_g_i))] \
                = get_V_hs_min_H(V_vent_g_i)
            V_hs_vent_d_t[C & (V_vent_g < get_V_hs_min_C(V_vent_g_i))] \
                = get_V_hs_min_C(V_vent_g_i)
        else:  # 最低風量直接入力なし
            # デフォルト条件 -> オリジナルのまま
    ...
    # 全般換気の機能を有さない場合
        ...
        if input_V_hs_min == 最低風量直接入力.入力する.value:  # 最低風量直接入力あり
            ...
            V_hs_vent_d_t[H] = get_V_hs_min_H(V_vent_g_i)
            V_hs_vent_d_t[C] = get_V_hs_min_C(V_vent_g_i)
        else:  # 最低風量直接入力なし
            # オリジナルのまま
            V_hs_vent_d_t = dc.get_V_hs_vent_d_t(V_vent_g_i, general_ventilation)
